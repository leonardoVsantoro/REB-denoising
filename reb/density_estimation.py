...
def density_estimate(manifold_type, X, M, on_X, grad=True, laplacian=False, normalise=False):
    """
    Spectral density estimate on compact Riemannian manifolds.

    Estimates the probability density f and (optionally) its Riemannian gradient
    and Laplacian using a spectral/projection approach.
    ...
    X : np.ndarray
        Data points on the manifold in extrinsic coordinates, shape (n, d).
    M : int
        Spectral truncation level (number of basis functions).
    on_X : np.ndarray
        Evaluation points in extrinsic coordinates.
    grad : bool, optional
        Whether to compute the Riemannian gradient. Default True.
    laplacian : bool, optional
        Whether to compute the Laplacian. Default False.
    ...
    -------
    on_X : np.ndarray
        Evaluation points (passed through).
    hat_f : np.ndarray
        Estimated density values at on_X.
    hat_grad_f : np.ndarray
        Estimated Riemannian gradient at on_X (only if grad=True).
    hat_lap_f : np.ndarray
        Estimated Laplacian at on_X (only if laplacian=True).
    """
...
def kernel_density_estimate(manifold_type, X, on_X, kappa):
    """
    Von Mises-Fisher kernel density estimate on S1 or S2.
    ...
    manifold_type : str
        'S1' or 'S2'.
    X : np.ndarray
        Data points in extrinsic coordinates, shape (n, d).
    on_X : np.ndarray
        Evaluation points in extrinsic coordinates.
    kappa : float
        Bandwidth (concentration parameter). Larger kappa = more localised kernel.
    ...
    hat_grad_f : np.ndarray
        Estimated gradient of the density.
    """
    n = X.shape[0]
    grid = on_X
    n_grid = grid.shape[0]
