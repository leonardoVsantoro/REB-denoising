...
def uniform_sampler(manifold_type, num_samples):
    """
    Sample uniformly from the given manifold.
    ...
    -------
    np.ndarray
        Uniform samples in extrinsic coordinates.
    """
    if manifold_type == 'S1':
...
def dirac_sampler(manifold_type, num_samples, n_points=1):
    """
    Sample from a discrete (Dirac mixture) prior on the manifold.

    Samples are drawn uniformly from n_points fixed support points.
    ...
    num_samples : int
    n_points : int
        Number of support points in the mixture.

    Returns
    -------
    np.ndarray
        Samples in extrinsic coordinates.
    """
    def _uniform_points(manifold_type, N):
        if manifold_type == "S1":
    ...
    points = _uniform_points(manifold_type, int(n_points))
...
def equator_sampler(manifold_type, num_samples, tau2=0.01):
    """
    Sample near an equator of S2 with small Riemannian Gaussian perturbation.
    ...
    num_samples : int
    tau2 : float
        Variance of the perturbation around the equator.

    Returns
    -------
    np.ndarray
        Points on S2 concentrated near the equator, shape (num_samples, 3).
    """
    if manifold_type != "S2":
...
def multimodal_sampler(manifold_type, n_samples, tau2, num_modes):
    """
    Sample from a multimodal (mixture of Riemannian Gaussians) prior.
